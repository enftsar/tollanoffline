...
function flagMarkup(country) {
  return `<span class="flag-mark">${flagFor(country)}</span>`;
}

function statusClass(status = "Pending review") {
  return status.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "");
}

function renderBracket(bracket) {
  bracketState = bracket;
  bracketGrid.innerHTML = `${bracket.champion ? `<div class="champion-banner">Champion: ${flagMarkup(bracket.champion.country)} ${escapeHtml(bracket.champion.name)} · ${escapeHtml(bracket.champion.country)}</div>` : ""}
    ${bracket.rounds
      .map(
        (round) => `<div class="bracket-round"><h3>${round.name}</h3>${round.matches
          .map(
            (match) => `<article class="bracket-match"><div class="bracket-status">${escapeHtml(match.label)}${match.deadline ? ` · Deadline ${escapeHtml(match.deadline)}` : ""}</div>${match.players
              .map((player, index) => `<div class="bracket-player ${match.winner === index ? "is-winner" : ""}"><strong>${flagMarkup(player.country)}${escapeHtml(player.name)}</strong><span>${escapeHtml(player.country)}</span></div>`)
              .join("")}</article>`,
          )
          .join("")}</div>`,
      )
...
function renderAdminBracketControls(bracket) {
  const playerPool = adminRecordings.length
    ? `<div class="player-pool"><strong>Submitted player pool</strong>${adminRecordings
        .map((item) => `<button type="button" data-pool-name="${encodeURIComponent(item.username || "Unnamed")}" data-pool-country="${encodeURIComponent(item.country || "")}">${flagMarkup(item.country)}${escapeHtml(item.username || "Unnamed")} · ${escapeHtml(item.country || "No country")}</button>`)
        .join("")}</div>`
    : `<div class="player-pool"><strong>Submitted player pool</strong><span>No submitted players yet.</span></div>`;
  adminBracketControls.innerHTML = `${playerPool}${bracket.rounds
    .flatMap((round) => round.matches.map((match) => ({ round: round.name, match })))
    .map(
      ({ round, match }) => `<div class="admin-bracket-match">
        <strong>${escapeHtml(round)} · ${escapeHtml(match.label)}</strong>
        <input data-edit-label="${match.id}" value="${escapeHtml(match.label)}" />
        <input data-edit-deadline="${match.id}" placeholder="Deadline" value="${escapeHtml(match.deadline || "")}" />
        <input data-edit-player="${match.id}" data-player-index="0" value="${escapeHtml(match.players[0].name)}" />
        <input data-edit-country="${match.id}" data-player-index="0" value="${escapeHtml(match.players[0].country)}" />
        <input data-edit-player="${match.id}" data-player-index="1" value="${escapeHtml(match.players[1].name)}" />
        <input data-edit-country="${match.id}" data-player-index="1" value="${escapeHtml(match.players[1].country)}" />
        <button class="rune-button ghost" type="button" data-save-match="${match.id}">SAVE</button>
        <button class="rune-button ghost" type="button" data-bracket-match="${match.id}" data-player-index="0">ADVANCE ${escapeHtml(match.players[0].name)}</button>
        <button class="rune-button ghost" type="button" data-bracket-match="${match.id}" data-player-index="1">ADVANCE ${escapeHtml(match.players[1].name)}</button>
      </div>`,
    )
    ...
    ? filtered
        .map(
          (item) => `<div class="admin-row">
            <div><strong>${escapeHtml(item.username || "Unnamed")}</strong><br><span>${flagMarkup(item.country)}${escapeHtml(item.country || "No country")}</span></div>
            <div>${escapeHtml(item.discord || "No Discord")}<br><span>${escapeHtml(item.wallet || "No wallet")}</span></div>
            <div>${escapeHtml(item.recordingId)}<br><span>${escapeHtml(item.sizeMb || "0.00")} MB · ${escapeHtml(item.metadata?.duration || "-")}</span></div>
            <div><span class="status-badge status-${statusClass(item.status)}">${escapeHtml(item.status || "Pending review")}</span><br><a href="${item.statusUrl}" target="_blank">STATUS</a> · <a href="${item.reviewUrl}" target="_blank">REVIEW</a></div>
            <a href="${item.videoUrl}" target="_blank" rel="noreferrer">WATCH</a>
            <div class="decision-buttons">
              <input class="note-input" data-note="${escapeHtml(item.recordingId)}" placeholder="Public/admin note" />
...
async function lookupStatus(id = statusLookupInput.value.trim()) {
  ...
  statusResult.innerHTML = `<strong>${escapeHtml(item.recordingId)}</strong><br>${escapeHtml(item.username)} · ${flagMarkup(item.country)}${escapeHtml(item.country)}<br><span class="status-badge status-${statusClass(item.status)}">${escapeHtml(item.status)}</span><br>${escapeHtml(item.adminNote || "")}`;
}
...
adminBracketControls.addEventListener("click", (event) => {
  ...
  const poolPlayer = event.target.closest("[data-pool-name]");
  if (poolPlayer) {
    const name = decodeURIComponent(poolPlayer.dataset.poolName);
    const country = decodeURIComponent(poolPlayer.dataset.poolCountry);
    const playerInputs = $$("[data-edit-player]");
    const target = playerInputs.find((input) => !input.value.trim() || input.value.trim().toUpperCase() === "TBD") || playerInputs[0];
    if (target) {
      target.value = name;
      const countryInput = $(`[data-edit-country="${target.dataset.editPlayer}"][data-player-index="${target.dataset.playerIndex}"]`);
